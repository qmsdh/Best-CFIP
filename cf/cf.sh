...
rm -rf 6.csv 4.csv
echo "秋名山转载于勇哥的脚本，并选择一个稳定的版本进行存档，脚本并非我创作！！！"
echo "甬哥GitHub ：https://github.com/yonggekkk"
echo "秋名山博客：blog.qmsdh.com"
echo
...
read -p "请选择【1-5】:" menu
if [ ! -e cf ]; then
curl -L -o cf -# --retry 2 --insecure https://raw.githubusercontent.com/qmsdh/Best_CFIP/main/cf/$cpu
chmod +x cf
fi
if [ ! -e locations.json ]; then
curl -s -o locations.json https://raw.githubusercontent.com/qmsdh/Best_CFIP/main/cf/locations.json
fi
if [ ! -e ips-v4.txt ]; then
curl -s -o ips-v4.txt https://raw.githubusercontent.com/qmsdh/Best_CFIP/main/cf/ips-v4.txt
fi
if [ ! -e ips-v6.txt ]; then
curl -s -o ips-v6.txt https://raw.githubusercontent.com/qmsdh/Best_CFIP/main/cf/ips-v6.txt
fi
if [ "$menu" = "1" ]; then
ip=4
